...
routes.get('/order/:order_id',function(req, res){
	...
	if (orderId.trim.length) {
		...
		models.instance.Order.find({id:orderId},function(err,order){
			if (err) {
				var status = '{"status":"error","message":"Problem fetching data from database."}';
				res.json(JSON.parse(status));
			} else {
				if(order.length!=1){
					var status = '{"status":"error","message":"Order improper."}';
					res.json(JSON.parse(status));
				} else {
					res.json(order[0]);
				}
...
routes.post('/order',function(req, res){
	...
	var order = new models.instance.Order({
		...
		links : {'pay':'http://myportal.com/pay','cancel':'http://myportal.com/cancel'},
		location : 'Satana row',
					...
					if ( order.status==='PLACED' ) {
						...
						models.instance.Order.update(query_obj, update_value,function(err) {
							if (err) {
								...
								models.instance.Order.findOne({id:orderId},function(err,ord){
									res.json(ord);
								});
							}
						});
					} else {
						var status = '{"status":"success","message":"Payment already made."}';
						res.json(JSON.parse(status));
					}
				}
			}
