...
func GetOrdersEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	var row OrderRow
	for rows.Next(&row) {
		orderURI := URI + row.Order.ID
		if *row.Order.Status == PLACED {
			row.Order.Links = &Links{Payment: orderURI + "/pay", Order: orderURI}
		} else {
			row.Order.Links = &Links{Order: orderURI}
		}
...
func GetOrderEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	if orderRow.Order.ID != "" {
		orderURI := URI + orderRow.Order.ID
		if *orderRow.Order.Status == PLACED {
			orderRow.Order.Links = &Links{Payment: orderURI + "/pay", Order: orderURI}
		} else {
			orderRow.Order.Links = &Links{Order: orderURI}
		}
...
func PlaceOrderEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	orderURI := URI + order.ID
	order.Links = &Links{Payment: orderURI + "/pay", Order: orderURI}
...
func UpdateOrderEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	if orderRow.Order.ID != "" {
		if *orderRow.Order.Status == PLACED {
			...
			orderURI := URI + orderRow.Order.ID
			newOrder.Links = &Links{Payment: orderURI + "/pay", Order: orderURI}
...
func main() {
	...
	if storeName != "" {
		URI = "http://" + GetLocalIP() + ":" + hostPort + "/" + storeName + "/v3/starbucks/order/"
	} else {
		URI = "http://" + GetLocalIP() + ":" + hostPort + "/v3/starbucks/order/"
	}

	router := mux.NewRouter()
	router.Methods("OPTIONS").HandlerFunc(IgnoreOption)
